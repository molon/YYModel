...
//ns pseudo-generic
@protocol NSString,NSMutableString,NSValue,NSNumber,NSDecimalNumber,NSData,NSMutableData,NSURL,NSDate,NSArray,NSMutableArray,NSDictionary,NSMutableDictionary,NSSet,NSMutableSet;
 ...
 * Implementations of `NSCoding`, `NSCopying`, `-hash` and `-isEqual:`.
 ...
 See `XXModel` protocol for custom methods.
 
 
 Sample Code:
    
     ********************** json convertor *********************
     interface XXAuthor : NSObject
     @property (nonatomic, strong) NSString *name;
     @property (nonatomic, assign) NSDate *birthday;
     @end
     implementation XXAuthor
     @end
 
     interface XXBook : NSObject
     @property (nonatomic, copy) NSString *name;
     @property (nonatomic, assign) NSUInteger pages;
     @property (nonatomic, strong) XXAuthor *author;
     @end
     implementation XXBook
     @end
    
     int main() {
         // create model from json
         XXBook *book = [XXBook xx_modelWithJSON:@"{\"name\": \"Harry Potter\", \"pages\": 256, \"author\": {\"name\": \"J.K.Rowling\", \"birthday\": \"1965-07-31\" }}"];
 
         // convert model to json
         NSString *json = [book xx_modelToJSONString];
         // {"author":{"name":"J.K.Rowling","birthday":"1965-07-31T00:00:00+0000"},"name":"Harry Potter","pages":256}
     }
 
     ********************** Coding/Copying/hash/equal *********************
     interface XXShadow :NSObject <NSCoding, NSCopying>
     @property (nonatomic, copy) NSString *name;
     @property (nonatomic, assign) CGSize size;
     @end
 
     implementation XXShadow
     - (void)encodeWithCoder:(NSCoder *)aCoder { [self xx_modelEncodeWithCoder:aCoder]; }
     - (id)initWithCoder:(NSCoder *)aDecoder { self = [super init]; return [self xx_modelInitWithCoder:aDecoder]; }
     - (id)copyWithZone:(NSZone *)zone { return [self xx_modelCopy]; }
     - (NSUInteger)hash { return [self xx_modelHash]; }
     - (BOOL)isEqual:(id)object { return [self xx_modelIsEqual:object]; }
     @end
 ...
 */
+ (nullable instancetype)xx_modelWithJSON:(id)json;
...
/**
 ...
 property, this method will try to convert the value based on these rules:
     ...
     `NSString` -> SEL, Class.
 */
+ (nullable instancetype)xx_modelWithDictionary:(NSDictionary *)dictionary;
...
/**
 ...
 Equals to xx_modelToJSONObjectOrRootSelf:NO
 */
- (nullable id)xx_modelToJSONObject;
 ...
 */
- (nullable NSData *)xx_modelToJSONData;
...
/**
 ...
 inner object to json string.
 */
- (nullable NSString *)xx_modelToJSONString;
 ...
 */
+ (nullable NSArray *)xx_modelArrayWithClass:(Class)cls json:(id)json;
 ...
 */
+ (nullable NSDictionary *)xx_modelDictionaryWithClass:(Class)cls json:(id)json;
@end
...
/**
 ...
 There's no need to add '<XXModel>' to your class header.
 */
...
/**
 ...
 object will be add to the array/set/dictionary.
  ...
  Example:
        class XXShadow, XXBorder, XXAttachment;
 
        interface XXAttributes
        @property NSString *name;
        ...
        implementation XXAttributes
        + (NSDictionary *)modelContainerPropertyGenericClass {
            return @{@"shadows" : [XXShadow class],
                     @"borders" : XXBorder.class,
                     @"attachments" : @"XXAttachment" };
        }
        @end
...
/**
 ...
 Example:
        class XXCircle, XXRectangle, XXLine;
 
        implementation XXShape

        + (Class)modelCustomClassForDictionary:(NSDictionary*)dictionary {
            if (dictionary[@"radius"] != nil) {
                return [XXCircle class];
            } else if (dictionary[@"width"] != nil) {
                return [XXRectangle class];
...
/**
 ...
 Example for _type:
 XXEncodingTypeIsCNumber ->@(-1)...
 XXEncodingTypeObject-> value with subclass of NSObject
 XXEncodingTypeClass -> [XXX class]
 XXEncodingTypeSEL -> NSStringFromSelector(@selector(test))
 XXEncodingTypeBlock -> just block
 
 XXEncodingTypePointer -> NSValue
 XXEncodingTypeStruct -> NSValue
 XXEncodingTypeUnion -> NSValue
 XXEncodingTypeCString -> NSValue
 XXEncodingTypeCArray -> NSValue
...
/**
 XXModelTransformProtocol is an abstract class which provides the
 basic structure for performing protocol-specific transforming of XXModel.
 Concrete subclass handles the specifics associated with one or more methods.
 */
@interface XXModelTransformProtocol : NSObject

/**
 This method registers a protocol class, making it visible to several other `XXModelTransformProtocol` class methods.
 
 @param cls A protocol class which must be subclass of `XXModelTransformProtocol` class.
 */
...
/**
 This method's behavior is similar to `+ (nullable NSDictionary<NSString *, id> *)modelCustomPropertyMapper;` of `XXModel` protocol. It returns the additional mapper finally.
...
/*!
 Change the value before transform from value, if returns nil, would use the original value.
 ...
 @return new value
 */
+ (nullable id)newValueBeforeTransformFromValue:(id)value modelClass:(Class)cls modelKey:(NSString*)modelKey;

/*!
 Change the model before transform from model, if returns nil, would use the original model.
 ...
 @return new model
 */
+ (nullable id)newModelBeforeTransformFromModel:(id)model modelKey:(NSString*)modelKey;
